...
export function ResultsTable({
  ...
  onInspectCandidates
      ...
      return (
        <div>
          <section className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded">
            <h4 className="mb-2 text-sm font-semibold text-slate-700">Column Normalization Status</h4>
            <ul className="list-disc ml-5 text-xs">
              {filtered.map((result, idx) => (
                <li key={idx}>
                  <span className="font-semibold">{result.sourceColumnName}</span>: <span className='text-slate-500'>none</span>
                </li>
              ))}
            </ul>
          </section>
